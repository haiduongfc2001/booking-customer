 ...
 */
export const extractSurchargeValues = (
  sortedSurchargeKeys: string[],
  surchargeRates: { [key: string]: number }
): {
  freeChildAge: string;
  freeChildMaxAge: number;
  nextChildRange: string;
  nextChildStartAge: number;
  nextChildEndAge: number;
  adultAge: string;
  adultStartAge: number;
  adultRate: number;
} => {
  const freeChildAge =
    sortedSurchargeKeys.find((ageRange) => surchargeRates[ageRange] === 0) ||
    "0-4";
  ...
  const nextChildRange =
    sortedSurchargeKeys.find(
      (ageRange) =>
        surchargeRates[ageRange] !== 0 && surchargeRates[ageRange] < 1
    ) || "5-12";
  const nextChildStartAge = parseInt(nextChildRange.split("-")[0]);
  const nextChildEndAge = parseInt(nextChildRange.split("-")[1]);

  const adultAge =
    sortedSurchargeKeys.find((ageRange) => surchargeRates[ageRange] === 0.5) ||
    "13-17";
